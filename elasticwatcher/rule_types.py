...
class RuleType(object):
    ...
    def __init__(self, rule, args=None):
        ...
        self.rule = rule
        self.occurrences = {}
        self.rule['owner'] = self.rule.get('owner', '')
        self.rule['priority'] = self.rule.get('priority', '2')
    ...
    def add_match(self, event):
        ...
        ts = self.rule.get('timestamp_field')
        if ts in event:
...
class ExpectedRule(CompareRule):
...
class ChangeRule(CompareRule):
    ...
    def compare(self, event):
        key = hashable(lookup_es_key(event, self.rule['query_key']))
        val = lookup_es_key(event, self.rule['compare_key'])
        if not val and self.rule['ignore_null']:
            return False
        changed = False
        ...
        if key in self.occurrences:
            ...
            if changed:
                ...
                if key in self.occurrence_time:
                    changed = event[self.rule['timestamp_field']] - self.occurrence_time[key] <= self.rule['timeframe']

        # Update the current value and time
        self.occurrences[key] = val
        if 'timeframe' in self.rule:
            self.occurrence_time[key] = event[self.rule['timestamp_field']]
    ...
    def add_match(self, match):
        ...
        change = self.change_map.get(hashable(lookup_es_key(match, self.rule['query_key'])))
        extra = {}
        if change:
...
class FrequencyRule(RuleType):
    ...
    def __init__(self, *args):
        ...
        self.attach_related = self.rule.get('attach_related', False)
    ...
    def add_count_data(self, data):
        ...
        self.occurrences.setdefault('all', EventWindow(self.rule['timeframe'], getTimestamp=self.get_ts)).append(event)
        ...
        for timestamp, buckets in terms.iteritems():
            for bucket in buckets:
                event = ({self.ts_field: timestamp,
                          self.rule['query_key']: bucket['key']}, bucket['doc_count'])
                self.occurrences.setdefault(bucket['key'], EventWindow(self.rule['timeframe'], getTimestamp=self.get_ts)).append(event)
    ...
    def add_data(self, data):
        ...
        for event in data:
            ...
            self.occurrences.setdefault(key, EventWindow(self.rule['timeframe'], getTimestamp=self.get_ts)).append((event, 1))
            self.check_for_match(key)

    def check_for_match(self, key):
        # Match if, after removing old events, we hit num_events
        if self.occurrences[key].count() >= self.rule['num_events']:
    ...
    def garbage_collect(self, timestamp):
        ...
        for key, window in self.occurrences.iteritems():
            if timestamp - lookup_es_key(window.data[-1][0], self.ts_field) > self.rule['timeframe']:
    ...
    def get_match_str(self, match):
        ...
        starttime = pretty_ts(dt_to_ts(ts_to_dt(match_ts) - self.rule['timeframe']), lt)
...
class SpikeRule(RuleType):
    ...
    def __init__(self, *args):
        ...
        self.timeframe = self.rule['timeframe']
    ...
    def add_terms_data(self, terms):
        for timestamp, buckets in terms.iteritems():
            for bucket in buckets:
                ...
                event = {self.ts_field: timestamp,
                         self.rule['query_key']: bucket['key']}
                key = bucket['key']
                self.handle_event(event, count, key)

    def add_data(self, data):
        for event in data:
            qk = self.rule.get('query_key', 'all')
            if qk != 'all':
    ...
    def clear_windows(self, qk, event):
        ...
        self.skip_checks[qk] = event[self.ts_field] + self.rule['timeframe'] * 2
    ...
    def handle_event(self, event, count, qk='all'):
        ...
        if event[self.ts_field] - self.first_event[qk][self.ts_field] < self.rule['timeframe'] * 2:
            ...
            if not (self.rule.get('query_key') and self.rule.get('alert_on_new_data')):
                return
    ...
    def find_matches(self, ref, cur):
        ...
        if (cur < self.rule.get('threshold_cur', 0) or
                ref < self.rule.get('threshold_ref', 0)):
            return False

        spike_up, spike_down = False, False
        if cur <= ref / self.rule['spike_height']:
            spike_down = True
        if cur >= ref * self.rule['spike_height']:
            spike_up = True

        if (self.rule['spike_type'] in ['both', 'up'] and spike_up) or \
           (self.rule['spike_type'] in ['both', 'down'] and spike_down):
            return True
        return False

    def get_match_str(self, match):
        message = 'An abnormal number (%d) of events occurred around %s.\n' % (match['spike_count'],
                                                                               pretty_ts(match[self.rule['timestamp_field']], self.rule.get('use_local_time')))
        message += 'Preceding that time, there were only %d events within %s\n\n' % (match['reference_count'], self.rule['timeframe'])
        return message
    ...
    def garbage_collect(self, ts):
        ...
        for qk in self.cur_windows.keys():
            ...
            if qk != 'all':
                placeholder.update({self.rule['query_key']: qk})
...
class FlatlineRule(FrequencyRule):
    ...
    def check_for_match(self, key):
        ...
        if most_recent_ts - self.first_event[key] < self.rule['timeframe']:
            return
    ...
    def garbage_collect(self, ts):
        ...
        for key in self.occurrences.keys():
            self.occurrences.setdefault(key, EventWindow(self.rule['timeframe'], getTimestamp=self.get_ts)).append(({self.ts_field: ts}, 0))
            self.check_for_match(key)
...
class NewTermsRule(RuleType):
    ...
    def __init__(self, rule, args=None):
        ...
        if 'fields' not in self.rule:
            if 'query_key' not in self.rule:
                raise EAException("fields or query_key must be specified")
            self.fields = self.rule['query_key']
        else:
            self.fields = self.rule['fields']
        if not self.fields:
            raise EAException("fields must not be an empty list")
        if type(self.fields) != list:
            self.fields = [self.fields]
        if self.rule.get('use_terms_query') and (
    ...
    def get_all_terms(self, args):
        ...
        self.es = Elasticsearch(
            host=self.rule['es_host'],
            port=self.rule['es_port'],
            timeout=self.rule.get('es_conn_timeout', 50),
            send_get_body_as=self.rule.get('send_get_body_as', 'GET')
        )
        window_size = datetime.timedelta(**self.rule.get('terms_window_size', {'days': 30}))
        ...
        step = datetime.timedelta(**self.rule.get('window_step_size', {'days': 1}))

        for field in self.fields:
            tmp_start = start
            tmp_end = min(start + step, end)

            time_filter = {self.rule['timestamp_field']: {'lt': dt_to_ts(tmp_end), 'gte': dt_to_ts(tmp_start)}}
            ...
            while tmp_start < end:
                if self.rule.get('use_strftime_index'):
                    index = format_index(self.rule['index'], tmp_start, tmp_end)
                else:
    ...
    def add_data(self, data):
        for document in data:
            for field in self.fields:
                ...
                if not value and self.rule.get('alert_on_missing_field'):
                ...
                if bucket['doc_count']:
                    if bucket['key'] not in self.seen_values[field]:
                        match = {field: bucket['key'],
                                 self.rule['timestamp_field']: timestamp,
                                 'new_field': field}
...
class CardinalityRule(RuleType):
    ...
    def __init__(self, *args):
        ...
        if 'max_cardinality' not in self.rule and 'min_cardinality' not in self.rule:
            raise EAException("CardinalityRule must have one of either max_cardinality or min_cardinality")
        self.ts_field = self.rule.get('timestamp_field', '@timestamp')
        self.cardinality_field = self.rule['cardinality_field']
        self.cardinality_cache = {}
        self.first_event = {}
        self.timeframe = self.rule['timeframe']

    def add_data(self, data):
        qk = self.rule.get('query_key')
        for event in data:
            if qk:
                key = hashable(lookup_es_key(event, qk))
            else:
    ...
    def check_for_match(self, key, event, gc=True):
        ...
        if (len(self.cardinality_cache[key]) > self.rule.get('max_cardinality', float('inf')) or
                (len(self.cardinality_cache[key]) < self.rule.get('min_cardinality', float('-inf')) and timeframe_elapsed)):
