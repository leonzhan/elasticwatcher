...
def intervalInSecond(intervalstring):
    """ interval is string format like 1m, 2h, return unit is second """

    interval_exc = "Bad interval format for {0}".format(intervalstring)

    interval_regex = re.compile("^(?P<value>[0-9]+)(?P<unit>[{0}])".format("".join(interval_dict.keys())))
    seconds = 0

    while intervalstring:
        match = interval_regex.match(intervalstring)
        if match:
            value, unit = int(match.group("value")), match.group("unit")
            if int(value) and unit in interval_dict:
                seconds += value * interval_dict[unit]
                intervalstring = intervalstring[match.end():]
            else:
